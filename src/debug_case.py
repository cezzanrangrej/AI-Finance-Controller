"""
Targeted debug script for AI Finance Controller.

Allows running an agent investigation on a single transaction exception with any provider
(OpenRouter, Gemini, or Demo), displaying detailed tool traces, timing, tokens,
and comparison with synthetic ground truth.

Usage:
    python src/debug_case.py --provider openrouter TXN003
    python src/debug_case.py TXN003
"""
...
def debug_case(transaction_id: str, provider: str = "openrouter", model: str = None) -> None:
    """Runs a targeted investigation on a specific transaction ID."""
    print("=" * 60)
    print(f"AI FINANCE CONTROLLER — {provider.upper()} CASE DEBUG ({transaction_id})")
    print("=" * 60)
    ...
    # 3. Instantiate client and AgentController
    ...
    print("\n" + "-" * 60)
    print(f"RUNNING {provider.upper()} INVESTIGATION...")
    print("-" * 60)
    ...
    if not log.tool_traces:
        ...
        for idx, trace in enumerate(log.tool_traces, 1):
            ...
            if trace.duplicate_call_prevented:
                print("  Deduplicated: YES (Cached result reused)")
            if trace.evidence_sufficient:
                print("  Evidence:   SUFFICIENT (Proof conditions satisfied)")
            if trace.early_stop_reason:
                print(f"  Early Stop: {trace.early_stop_reason}")
    ...
    tokens_used = getattr(llm_client, "cumulative_total_tokens", None)
    tok_str = f"{tokens_used:,}" if tokens_used else "N/A"
    ...
    print(f"Investigation Time:  {elapsed:.4f}s")
    print(f"Tokens Used:         {tok_str}")
    print("-" * 60)
...
def main():
    parser = argparse.ArgumentParser(description="Debug an exception case with an LLM provider.")
    parser.add_argument("transaction_id", type=str, help="Transaction ID to debug (e.g. TXN003)")
    parser.add_argument("--provider", type=str, default=os.getenv("LLM_PROVIDER", "openrouter"), choices=["openrouter", "gemini", "demo"])
    parser.add_argument("--model", type=str, default=None, help="Model override")
    args = parser.parse_args()

    debug_case(args.transaction_id.strip(), provider=args.provider, model=args.model)
