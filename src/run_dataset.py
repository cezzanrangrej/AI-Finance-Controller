...
def run_llm_mode(
    ...
    if mode == "batch":
        ...
        for idx, exc in enumerate(selected, 1):
            txn_id = exc.get("transaction_id", "UNKNOWN")
            reason = exc.get("reason", "UNKNOWN")
            print(f"  [{idx}/{len(selected)}] Investigating {txn_id} ({reason}) [Multi-Agent]...")
            ...
            inv = getattr(log, "investigator_calls", "?")
            ver = getattr(log, "verifier_calls", "?")
            print(f"    -> Decision: {tag} | Inv calls: {inv}, Ver calls: {ver}")
