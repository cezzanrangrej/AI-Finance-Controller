...
# ----------------------------------------------------------------------
# Test 1: Investigator uses its own role-scoped provider
# ----------------------------------------------------------------------
def test_investigator_uses_role_scoped_provider(sample_toolkit):
    with patch.dict(os.environ, {
        ...
        orch = MultiAgentOrchestrator(toolkit=sample_toolkit, provider="gemini")
        assert isinstance(orch.investigator_llm, OpenRouterLLMClient)
        assert orch.investigator_llm.model == "meta-llama/llama-3.3-70b-instruct"
...
def test_role_specific_provider_overrides_default(sample_toolkit):
    with patch.dict(os.environ, {
        ...
        # provider="gemini" is the base; both roles name their own, so both win.
        orch = MultiAgentOrchestrator(toolkit=sample_toolkit, provider="gemini")
        assert isinstance(orch.investigator_llm, OpenRouterLLMClient)
        assert isinstance(orch.verifier_llm, GeminiLLMClient)
...
# ----------------------------------------------------------------------
# Test 5: Missing OpenRouter key fails clearly
# ----------------------------------------------------------------------
...
def test_api_key_as_model_id_raises_configuration_error():
    with pytest.raises(ValueError, match="ConfigurationError: OPENROUTER_MODEL appears to contain a credential"):
        validate_model_not_key("openrouter", "sk-or-v1-0000000000000000000000000000000000000000")
...
def test_api_key_never_appears_in_trace():
    ...
    tracer.agent_started("Investigator", "OPENROUTER", "meta-llama/llama-3.3-70b-instruct")
    tracer._print("Trace logging with key sk-or-v1-secret1234567890 and AIzaSySecret12345678901234567890")

    output = buf.getvalue()
    assert "sk-or-v1-secret" not in output
    assert "AIzaSySecret" not in output
...
def test_model_interaction_counter_success_and_failure(sample_toolkit):
    ...
    mock_llm_success.provider_name = "openrouter"
    mock_llm_success.model = "meta-llama/llama-3.3-70b-instruct"
    ...
    mock_llm_fail.provider_name = "openrouter"
    mock_llm_fail.model = "meta-llama/llama-3.3-70b-instruct"
...
def test_no_verifier_call_when_investigator_fails(sample_toolkit):
    ...
    mock_inv_llm.provider_name = "openrouter"
    mock_inv_llm.model = "meta-llama/llama-3.3-70b-instruct"
